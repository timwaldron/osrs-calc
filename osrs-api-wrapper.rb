...
class OSRS_Api_Wrapper
    ...
    def is_api_response_valid?(url_to_check) # Check the HTTP Response of an URL (2xx OK, anything else probably not ok)
    ...
    def get_item_data(item_id) # Gets all API data associated with an item on the Grand Exchange
        item_id = item_id.to_s # Conversion from Integer to string as RuneScape's item id convention has always been integers.
        if (is_api_response_valid?(@grandexchange_base_url + item_id)) # If we get a successful response from this URL
    ...
    def get_item_price(item_id) # Gets the current price of an item from the Grand Exchange
        item_id = item_id.to_s # Conversion from Integer to string as RuneScape's item id convention has always been integers.
        if (is_api_response_valid?(@grandexchange_base_url + item_id)) # If we get a successful response from this URL
            ...
            item_price.gsub!(",", "") # ',' is used between numbers 1,000 and 9,999 it will generally be displayed 
            ...
            if (!item_price.include?(".")) # If item_price doesn't include '.'
                ...
                item_price.gsub!(".", "") # '.' only returned in the number if it's above 10k and not clean (55.7k, not 55k) 
    ...
    def get_hiscore_data(in_game_name) # Gets hiscore data associated with a player 
        if (is_api_response_valid?(@hiscore_base_url + in_game_name)) # If we get a successful response from this URL
            return download_hiscore_data(@hiscore_base_url + in_game_name) # Download and return the players data
        else 
            return false # otherwise return false
        end
    end

    def download_item_data(grand_exchange_item_url) # Downloads items Grand Exchange data
        ...
        return item_data_hash # returns the hash
    end
    
    def download_hiscore_data(hiscore_url) # Download players hiscore data
        ...
        return player_data # Return our hash of player_data
    end
end