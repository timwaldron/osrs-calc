...
module OSRS_Api_Wrapper
    ...
    def self.get_item_data(item_id) # Gets all API data associated with an item on the Grand Exchange
        ...
        if (self.is_http_response_valid?(@grandexchange_base_url + item_id)) # If we get a successful response from this URL
            return self.download_item_data(@grandexchange_base_url + item_id) # Download and return the item data
        else
            return false # Otherwise return 'false' because we can't be certain the request was a success
        end
    end
    ...
    def self.get_item_price(item_id) # Gets the current price of an item from the Grand Exchange
        item_id = item_id.to_s # Conversion from Integer to string as RuneScape's item id convention has always been integers.
        if (self.is_http_response_valid?(@grandexchange_base_url + item_id)) # If we get a successful response from this URL
            item_data_hash = self.download_item_data(@grandexchange_base_url + item_id) # Download and return the item data into a hash
    ...
    def self.get_hiscore_data(in_game_name) # Gets hiscore data associated with a player 
        if (self.is_http_response_valid?(@hiscore_base_url + in_game_name)) # If we get a successful response from this URL
            return self.download_hiscore_data(@hiscore_base_url + in_game_name) # Download and return the players data
        else 
